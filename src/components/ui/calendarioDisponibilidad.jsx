...
function CalendarioDisponibilidad({
  ...
  const categorizeDays = (reservationsByDay) => {
    ...
    Object.entries(reservationsByDay).forEach(([dateString, reservations]) => {
      const [day, month, year] = dateString.split("/");
      const date = new Date(year, month - 1, day);

      if (reservations >= 0 && reservations < 10 || reservations === undefined || reservations === null) {
        emptyDays.push(date);
  ...
  const { emptyDays, partiallyFilledDays, fullyFilledDays } = categorizeDays(
    reservationsByDay
  );
