...
const AgregarCanchaModal = ({ isOpen, onClose, onAdd }) => {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
   ...
   const { data: nuevaCancha, error: errorInsertCancha } = await supabase
    .from('Cancha')
    .insert([
     {
      Nombre: nombre,
  ...
  <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center mt-20 overflow-auto">
   ...
   >
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
     ...
     <div>
      <label className="block mb-1 text-base">Precio por hora:</label>
      <input
       type="number"
       value={precioHora}
     ...
     <div>
      <label className="block mb-1 text-base">Disciplina:</label>
      <select
       value={disciplinaId}
     ...
     <div>
      <label className="block mb-1 text-base">Tipo de superficie:</label>
      <select
       value={superficieId}
      ...
      >
       ...
       {superficies.map((superficie) => (
        <option key={superficie.id} value={superficie.id}>
         {superficie.Nombre}
        </option>
       ))}
      </select>
     ...
     <div>
      <span className="block mb-1 text-base">Características:</span>
     ...
     <div>
      <label className="block mb-1 text-base">Imágenes:</label>
      <input
       type="file"
       multiple
